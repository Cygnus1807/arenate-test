...
const TEAM_FIELDS =
  'id, event_id, created_by, name, description, min_size, max_size, open_to_join, status, created_at, updated_at';
const MEMBER_FIELDS = 'team_id, user_id, role, status, request_note, joined_at, profiles:profiles(full_name)';
...
export const joinTeam = async ({ teamId, userId, note }) => {
  ensureSupabase();
  const { data: team, error: teamError } = await supabase
    .from('teams')
    .select('id, max_size, min_size, open_to_join, status, members:team_members(status, user_id)')
    .eq('id', teamId)
    .single();
  if (teamError) {
  ...
  const { error } = await supabase
    .from('team_members')
    .upsert(
      { team_id: teamId, user_id: userId, status: 'pending', request_note: note ?? null },
